...
export const StudyCard: React.FC<StudyCardProps> = ({ title, helperText, children, className = "", onClick }) => {
  return (
      ...
      >
        <div className="mb-6 flex justify-between items-start">
          <div className="max-w-[85%]">
            <h3 className="text-navy text-[10px] font-bold tracking-[0.25em] uppercase font-sans mb-2 opacity-60">{title}</h3>
            {helperText && <p className="text-gray-400 text-xs italic font-serif leading-relaxed">{helperText}</p>}
          </div>
          {onClick && (
        ...
        <div className="text-charcoal selection:bg-gold/10">
          {children}
        </div>
      </div>
    </div>
